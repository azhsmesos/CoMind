...
也可设置 `F1`；Mac 部分键盘需要按 **Fn+F1**。输入 `Fn+F1` 时应用保存为 `F1`，是否需要 Fn 由系统键盘设置决定。若它打开了其他工具的截图界面，请更换组合键，或先解除其他工具的占用。CoMind 的截图界面只保留选框和左下角操作提示；其他工具产生的截图不会自动上传到本应用。
...
截图只需按快捷键、拖动框选、松开鼠标，随后自动上传。框选窗口在屏幕图片解码并绘制好后显示，不使用全屏变暗遮罩；右键或 Esc 取消。

快捷键页面的“截图日志”默认折叠，终端和页面仅显示开始、上传、完成或错误等主要状态。权限、屏幕读取、选区等详细诊断保存在页面所示的本地 `screenshot.log`，超过约 1 MB 时轮换，保留一份旧日志；不记录截图内容或模型密钥。“最近收到截图快捷键”仅表示按键已触发，不代表框选或识别已完成。
