...
async function resolvePort(hint) {
  ...
  for (const port of candidates) {
    ...
    try {
      ...
      if (res.ok && (await res.json()).app === "comind") return port;
    } catch {
...
export async function pair() {
  ...
  if (port == null)
    return { ok: false, error: "桌面应用未启动，请先打开CoMind。" };
  try {
...
async function deliver(pairing) {
  ...
  if (!page || !page.text)
    return {
      ok: false,
      error: "页面没有可读取的文字，请等待加载完成或手动粘贴题目。",
    };
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  (async () => {
    if (msg?.type === "pair") sendResponse(await pair());
      ...
      const result = await sendActiveTab();
      if (result.ok) await chrome.storage.session.remove("lastError");
      else
        await chrome.storage.session.set({
          lastError: { at: Date.now(), error: result.error },
        });
      ...
      if (port && pairing) {
        try {
          paired = (
            await fetch(
              `http://127.0.0.1:${port}/status?t=${encodeURIComponent(pairing.token)}`,
              { signal: AbortSignal.timeout(2000) },
            )
          ).ok;
        } catch {}
      }
      sendResponse({ paired, running: !!port, lastError: lastError ?? null });
    }
  })();
